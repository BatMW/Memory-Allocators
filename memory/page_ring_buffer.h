...
Page_Ring_Buffer page_ring_buffer_init(size_t physical_nr_pages);

void page_ring_buffer_destroy(Page_Ring_Buffer* allocator);
...
void page_ring_buffer_destroy(Page_Ring_Buffer* buf) {
  if (!buf || !buf->base) return;
  for (int i = 0; i < 3; ++i) {
    VirtualFree((char*)buf->base + i * buf->p_size, buf->p_size, RELEASE);
  }
  VirtualFree(buf->base, 0, RELEASE);
  if (buf->hMap) {
