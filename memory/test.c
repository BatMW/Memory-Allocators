...
void test_reset_allocator(void){
  ...
  struct Reset_Allocator allocator = reset_allocator_init(mem, alloc_size);
  assert(allocator.base != 0);

  char* alloc1 = (char*)reset_allocator_alloc(&allocator, 512);
  assert(alloc1 != NULL);

  char* alloc2 = (char*)reset_allocator_alloc(&allocator, 600);
  assert(alloc2 == NULL);
  assert(allocator.failed_allocs == 1);

  reset_allocator_reset(&allocator);
  char* alloc3 = (char*)reset_allocator_alloc(&allocator, 256);
  assert(alloc1 == alloc3);
  assert(allocator.failed_allocs == 0);

  free(mem);
  reset_allocator_destroy(&allocator);
...
void test_ring_allocator(void){
  ...
  size_t nr_blocks = 4;
  void* mem = aligned_alloc(block_size, nr_blocks);
  struct Ring_Block_Allocator allocator = ring_block_allocator_init(mem, block_size, nr_blocks);
  assert(allocator.base != (uintptr_t)NULL);

  char* alloc0 = (char*)ring_block_allocator_alloc(&allocator);
  assert(alloc0 == (char*)allocator.base);

  char* alloc1 = (char*)ring_block_allocator_alloc(&allocator);
  assert(alloc1 != NULL);

  char* alloc2 = (char*)ring_block_allocator_alloc(&allocator);
  assert(alloc2 != NULL);

  char* alloc3 = (char*)ring_block_allocator_alloc(&allocator);
  assert(alloc3 != NULL);

  char* alloc4 = (char*)ring_block_allocator_alloc(&allocator);
  assert(alloc4 == NULL); //Full buffer

  ring_block_allocator_free(&allocator, alloc0);
  assert(allocator.tail == 1);

  ring_block_allocator_free(&allocator, alloc1);
  assert(allocator.tail == 2);

  ring_block_allocator_free(&allocator, alloc2);
  assert(allocator.tail == 3);

  alloc4 = (char*)ring_block_allocator_alloc(&allocator);
  assert(alloc4 == (char*)allocator.base);
  //printf("alloc4ptr: %p , baseptr: %p \n", (void*)(alloc4-1), allocator.base);
  ring_block_allocator_destroy(&allocator);
  free(mem);
  printf("Ring allocator test done.\n");
}

int check_ll(struct Pool_Allocator* allocator){
  printf("check linked list...\n");
  struct Pool_Memory_Block* checker = allocator->head;
  int sum=0;
  while(true){
    if(checker == NULL){
      break;
...
void test_pool_allocator(void){
  ...
  void* mem = aligned_alloc(64, 128);
  struct Pool_Allocator allocator = pool_allocator_init(mem, block_size, nr_blocks);
  assert(allocator.base != (uintptr_t)NULL);
  ...
  for(unsigned i=0; i<nr_blocks; ++i){
    ptrs[i] = pool_allocator_alloc(&allocator);
    assert(ptrs[i] != NULL);
    assert((size_t)ptrs[i] % block_size == 0);
  }
  //No more memory
  char* fail = pool_allocator_alloc(&allocator);
  assert(fail == NULL);


  char* old_spot_3 = ptrs[3];
  assert(pool_allocator_free(&allocator, ptrs[3]));
  ptrs[3]=NULL;
  ptrs[3] = pool_allocator_alloc(&allocator);
  assert(old_spot_3 == ptrs[3]);
  
  printf("Freeing all ...\n");
  for(unsigned i=0; i<nr_blocks; ++i){
    assert(pool_allocator_free(&allocator, ptrs[i]));
    ptrs[i] = NULL;
  }
  ...
  for(size_t i = 0; i< 10000000; ++i){
    ...
    if(ptrs2[index] == NULL){
      if(allocator.head == NULL){
        ptrs2[index] = pool_allocator_alloc(&allocator);
      ...
      assert(pool_allocator_free(&allocator, ptrs2[index]));
      ptrs2[index]=NULL;
    }
  }
  printf("Freeing all ...\n");
  for(size_t i=0; i<nr_blocks*2; ++i){
    pool_allocator_free(&allocator, ptrs2[i]);
    ptrs2[i] = NULL;
  }
  size_t sum = check_ll(&allocator);
  printf("Sum of linked ptrs: %zu\n nr blocks: %zu\n", sum, nr_blocks );
   assert(sum == nr_blocks);
  //assert(pool_allocator_destroy(&allocator));
   free(mem);
...
void test_stack_allocator(void){
  ...
  struct Stack_Allocator allocator = stack_allocator_init(mem, 1024*1024); //1 MiB
  assert(allocator.base != (uintptr_t)NULL);
...
void test_page_ring_buffer(void){
  ...
  fflush(stdout);
  struct Page_Ring_Buffer buf = page_ring_buffer_init(1);
